...
Car24 has `r nrow(cars24[!is.na(cars24$fitnessUpto), ])` rows with  `fitnessUpto` column whereas Carwale has `r nrow(carwale[!is.na(carwale$fitnessUpto), ])` rows with valid `fitnessUpto` values.
...
Car24 has `r length(unique(cars24$city))` values for `city` column whereas Carwale has `r length(unique(carwale$city))` values. They are as follows:
...
Car24 has `r length(unique(cars24$registrationYear))` values for `registrationYear` column whereas Carwale has `r length(unique(carwale$registrationYear))` values. They are as follows:
...
ggplot() + 
  ...
  labs(title = "Most used cars are 5-7 years olf registrations",
       subtitle = "Year-wise availability between May 2024 till Sep 2024",
       y = "Number of Used Cars",
       x = "Year") + 
  theme_minimal() +
